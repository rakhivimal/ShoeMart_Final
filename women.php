...
if($_POST["sort"]=="price"){
 $sql = "select * from product where category = 'women' order by model_price";
}
 elseif($_POST["sort"]=="size"){
$sql = "select * from product where category = 'women' order by size";
}
else{
...
?>
    ...
    <tr><td> <input name="checkbox[]" type="checkbox" id = "checkbox" value="<?php echo $row['model_id']; ?>"></td>
   <td> <img src="assets/shoe_img/<?php echo $row["image"]; ?>" class="border"><br></td>
         <td>  <?php echo "<b>Model:</b> " . $row["model_name"]."<br>";
			  ...
			  <!--<input type="submit" name ="submit" value="Add to Cart"></form> <br>-->
			  <input type='hidden' name="reviews" value="<?php echo $row['model_id']; ?>"></input>
			  <input type="submit" name ="review_submit" value="Reviews"></input></form></td></tr>
</table>
<?php
}
mysqli_close($conn);
